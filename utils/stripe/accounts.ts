	...
	try {
		...
		if (businessType === "individual") {
			accountData = {
				...
				tos_acceptance:
					country === "US"
						? { date: Math.floor(Date.now() / 1000), ip: userIp, user_agent: userAgent ?? "unknown" }
						: country === "CR"
							? { date: Math.floor(Date.now() / 1000), ip: userIp, service_agreement: "recipient" }
							: { date: Math.floor(Date.now() / 1000), ip: userIp },
				business_profile: {
			...
			accountData = {
				country,
				business_type: businessType as "company" | "non_profit",
				capabilities: { transfers: { requested: true } },
				controller: {
					stripe_dashboard: { type: "none" },
					fees: { payer: "application" },
				...
				company: {
					...
					...(businessType === "company" && { owners_provided: true }),
				},
			};
		}
