...
interface BloquearHorarioProps {
...
export const BloquearHorario: FC<BloquearHorarioProps> = ({ actividad }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        fecha_desde: formatDateToYYYYMMDD(dateRange.startDate),
        fecha_hasta: formatDateToYYYYMMDD(dateRange.endDate),
        tipo_modificacion: "BLOQUEAR_HORARIO",
        actividad_id: actividad?.id,
        horario_bloquear: horarioSeleccionado.hora,
      };
        ...
        {loading ? (
          <div className="flex flex-col gap-4 rounded-[8px] p-4">
            <div>
              ...
              >
                Selecciona el horario que deseas bloquear temporalmente
              </Typography>
            </div>
                                ...
                                {group.schedules.map((schedule) =>
                                  group.days[0] !== undefined ? (
                                    <Chip
                                      ...
                                      color={
                                        horarioSeleccionado?.id === schedule.id
                                          ? "primary"
                                          : "default"
                                      }
                                      variant={
