...
const BUCKET_NAME = "adicionales.imagenes";
 ...
 * @returns URL de la imagen guardada o null si hay error
 */
...
/**
 * Obtiene la URL de una imagen asociada a un adicional
 * @param adicionalId ID del adicional
 * @returns URL de la imagen o null si hay error
 */
export async function getImageUrl(adicionalId: number) {
  const supabase = await getSupabase();
...
/**
 * Actualiza la imagen asociada a un adicional
 * @param adicionalId ID del adicional
...
export async function updateImage(
  ...
  // Obtener la url de la imagen actual desde la base de datos
  ...
  if (adicionalError || !adicional) {
    logError(adicionalError || new Error("Adicional no encontrado"), {
      service: "imagenService",
      method: "updateImage",
      adicionalId,
    });
  ...
  const oldFilePath = adicional.imagen;
  ...
  logInfo("Imagen actualizada exitosamente", { adicionalId, newFilePath });
  return true;
}

/**
 * Elimina la imagen asociada a un adicional
 * @param adicionalId ID del adicional
 * @returns true si se eliminó correctamente, false en caso contrario
 */
export async function deleteImage(adicionalId: number) {
  const supabase = await getSupabase();
  ...
  const filePath = adicional.imagen.substring(
    adicional.imagen.lastIndexOf("/") + 1
  );
  ...
  logInfo("Imagen eliminada exitosamente", { adicionalId });
  return true;
}
